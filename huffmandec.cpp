...
void buildTree(node * mynode, string k, char m) {
  if (k.size() < 1) {
    mynode->value = m;
  }
  else {
    if (k[0] == '0') {
      if (mynode->zero == NULL) {
	mynode->zero = new node();
	buildTree(mynode->zero,k.substr(1),m);
      }
      else {
	buildTree(mynode->zero,k.substr(1),m);
      }
       ...
       if (mynode->one == NULL) {
	  mynode->one = new node();
	  buildTree(mynode->one,k.substr(1),m);
      }
      else {
	buildTree(mynode->one,k.substr(1),m);
      }
...
string decode(node * a,string j) {
  string message("");
  
  while (j.size() > 0) {
  //will need another one to go until all code is decoded
   node * temp = a;
   while (!(temp->zero == NULL && temp->one == NULL)) {
    if (j[0] == '0') {
      temp = temp->zero;
      j = j.substr(1);
    }
...
int main (int argc, char **argv) {
    ...
    string dec("");
    dec.append(decode(root,allbits));
    cout << dec << endl;
